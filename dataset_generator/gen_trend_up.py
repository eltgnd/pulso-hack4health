...
# Generate outcomes with a non-linear increasing trend
# Using an exponential function to simulate the increase
probability_increase = np.exp(np.linspace(0, 3, len(timestamps))) / (np.exp(3))

# Ensure outcomes follow the trend
outcomes = [1 if random.random() < p else 0 for p in probability_increase]
